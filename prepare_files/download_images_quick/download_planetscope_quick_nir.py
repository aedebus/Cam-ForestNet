...
def getResult(index, point, year, path):
  ...
  if os.path.exists(os.path.join(path, str(index), 'planet_nir')) == False:
      os.mkdir(os.path.join(path, str(index), 'planet_nir'))

  filename = os.path.join(path, str(index), 'planet_nir', str(year)+'_'+str(x)+'_'+ str(y)+'_'+str(index)+'.png') #remove: str(index)
