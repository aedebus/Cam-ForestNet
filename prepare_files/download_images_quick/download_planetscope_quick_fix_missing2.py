...
def getResult(index, point, year, path):
  ...
  image_coll = (ee.ImageCollection("projects/planet-nicfi/assets/basemaps/africa")
           .filterBounds(region)
           .filter(ee.Filter.eq('cadence', 'monthly')) #monthly
           .filterDate(str(year+1)+'-01-01', str(year+5)+'-12-31'))
  image_list = image_coll.toList(image_coll.size())
  image = ee. Image(image_list.get(1)).clip(region).select('R', 'G', 'B').visualize(min=64, max=5454, gamma=1.8)
  ...
  if os.path.exists(os.path.join(path, str(index), 'planet_fixed')) == False:
      os.mkdir(os.path.join(path, str(index), 'planet_fixed'))
      filename = os.path.join(path, str(index), 'planet_fixed', str(year)+'_'+str(x)+'_'+ str(y)+'_'+str(index)+'.png') #remove: str(index)
      with open(filename, 'wb') as out_file:
        shutil.copyfileobj(r.raw, out_file)
      print("Done: ", index)

  elif len(os.listdir(os.path.join(path, str(index), 'planet_fixed'))) == 0:
    filename = os.path.join(path, str(index), 'planet_fixed', str(year)+'_'+str(x)+'_'+ str(y)+'_'+str(index)+'.png') #remove: str(index)
    with open(filename, 'wb') as out_file:
       shutil.copyfileobj(r.raw, out_file)
    print("Done: ", index)
...
def planetscope_from_gfc():
    ...
    print('init ok')
    with open("to_redownload", "rb") as fp:   
        to_rd = pickle.load(fp)
    for i in range (len(to_rd)):
        to_rd[i][1] = ee.Geometry.Point([float(to_rd[i][1][0]), float(to_rd[i][1][1])])
    items = to_rd
...
def main():
    fire.Fire(planetscope_from_gfc) #after clean up
