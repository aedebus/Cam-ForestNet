...
def getResult(index, point, year, path):
  ...
  if (len(filtered_images.getInfo()['features'])>0):
      image = (ee.ImageCollection("LANDSAT/LC08/C02/T1_TOA")
               ...
               .first()
               .clip(region)
               .select('B5','B4', 'B3', 'B2')).visualize(min=0.0, max=0.4) # all bands
      ...
      if os.path.exists(os.path.join(path, str(index), 'landsat_nir_ps')) == False:
          os.mkdir(os.path.join(path, str(index), 'landsat_nir_ps'))

      filename = os.path.join(path, str(index), 'landsat_nir_ps', str(year)+'_'+str(x)+'_' + str(y)+'.png')
