...
def getResult(index, point, year, path):
  ...
  if (len(filtered_images.getInfo()['features'])>0):
      ...
      if os.path.exists(os.path.join(path, str(index), 'landsat_nir')) == False:
          os.mkdir(os.path.join(path, str(index), 'landsat_nir'))

      filename = os.path.join(path, str(index), 'landsat_nir', str(year)+'_'+str(x)+'_' + str(y)+'.png')
