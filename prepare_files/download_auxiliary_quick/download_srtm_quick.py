...
def getRequests(year):
  ...
  for shapes in list_dir:
        if ('production_forest' not in shapes) and ('small_scale_agriculture' not in shapes) and ('timber_sales' not in shapes) and ('mining_permits' not in shapes):
            ...
            for index in list_index:
                ...
                for sensor in list_sensors:
                    if sensor == 'landsat' or sensor == 'landsat_nir_ps':
                        ...
                        if sensor == 'planet_fixed': 
                            ...
                            if os.path.exists(os.path.join(path_out_shapes, str(index), 'landsat', str(centroid_x) + '_' + str(centroid_y) + '_' + 'altitude.tif')) == False:
                                list_points.append((index, ee.Geometry.Point([centroid_x, centroid_y]), year, os.path.join(path_out_shapes), 'landsat'))
                        else: 
                            if os.path.exists(os.path.join(path_out_shapes, str(index), sensor, str(centroid_x) + '_' + str(centroid_y) + '_' + 'altitude.tif')) == False or (os.path.exists(os.path.join(path_out_shapes, str(index), sensor, str(centroid_x) + '_' + str(centroid_y) + '_' + 'altitude.tif')) == True and os.path.getsize(os.path.join(path_out_shapes, str(index), sensor, str(centroid_x) + '_' + str(centroid_y) + '_' + 'altitude.tif'))==0):
...
def getResult(index, point, year, path, sensor):
  ...
  if sensor == 'landsat':
      ...
      param = (0.05 * ((332 * 10) / 30)) / 372
      region = ee.Geometry.BBox(lon - param, lat - param, lon + param, lat + param)
  elif sensor == 'sentinel2_20m':
      param = (0.05 * ((332 * 20) / 30)) / 372
      region = ee.Geometry.BBox(lon - param, lat - param, lon + param, lat + param)
  elif sensor == 'landsat_30':
      param = (0.05 * ((332 * 30) / 30)) / 372
      region = ee.Geometry.BBox(lon - param, lat - param, lon + param, lat + param)
  else:
...
def convert_srtm(year):
    ...
    for shapes in list_shapes:
        if ('production_forest' not in shapes) and ('small_scale_agriculture' not in shapes) and ('timber_sales' not in shapes):
