...
def populate_folder(folder, source, sensor, csv_file, merge_scheme):
    ...
    for year in years:
        ...
        for d in list_dir:
            if ('production_forest' not in d) and ('small_scale_agriculture' not in d) and ('timber_sales' not in d) and ('Subsistence' not in d): #to reduce files copied # added 
                indices = os.listdir(os.path.join(path_image, d))
                for i in indices:
                    #1. Obtain images
                    
                    images_source = os.path.join(path_image, d, i, sensor +'_sr_nir')
                    if os.path.exists(images_source) == True:
                        ...
                        ir_source = os.path.join(path_auxiliary, d, i, sensor + '_30')
                        #print(ir_source)
                        ...
                        aux_source = os.path.join(path_auxiliary, d, i, sensor + '_30')
                        #print(aux_source)
