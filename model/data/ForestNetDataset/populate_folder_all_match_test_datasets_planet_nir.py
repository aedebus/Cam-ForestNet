...
def get_split_indices(all_meta, train_split, val_split, merge_scheme):
    landsat = pd.read_csv(os.path.join(os.getcwd(), 'Test with Cameroon data (final)', 'Landsat final versions', 'Without ANY duplicates and errors', 'nir', 'test.csv'), names=CAMEROON_META_COLNAMES)
    ...
    landsat['merged_label']=landsat['merged_label'].astype(str)
    landsat['label']=landsat['label'].astype(str)

    planet_all['year']=planet_all['year'].astype(int)
    planet_all['longitude']=planet_all['longitude'].astype(float)
    planet_all['latitude']=planet_all['latitude'].astype(float)
    planet_all['merged_label']=planet_all['merged_label'].astype(str)
    planet_all['label']=planet_all['label'].astype(str)
