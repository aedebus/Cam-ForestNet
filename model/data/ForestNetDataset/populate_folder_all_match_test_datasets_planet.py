...
def get_split_indices(all_meta, train_split, val_split, merge_scheme):
    landsat = pd.read_csv(os.path.join(os.getcwd(), 'Test with Cameroon data (final)', 'Landsat final versions', 'test.csv'), names=CAMEROON_META_COLNAMES)
...
def populate_folder(folder, source, sensor, csv_file, merge_scheme):
    ...
    if len(os.listdir(os.path.join(os.path.join(os.getcwd(), folder)))) == 0:
        no_need_to_fill = False
    else:
        no_need_to_fill = True
        ...
        for d in list_dir:
            if ('production_forest' not in d) and ('small_scale_agriculture' not in d) and ('timber_sales' not in d): #to reduce files copied # added 
                ...
                for i in indices:
                    #1. Obtain images
                    if sensor == 'planet': 
                        images_source = os.path.join(path_image, d, i, sensor + '_fixed') #when fixing images
                    else:
                    ...
                    if csv_to_fill or no_need_to_fill:
                        list_data=[]
    ...
    # 6. Split data and random stratified sampling
