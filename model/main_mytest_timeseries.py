...
def test(ckpt_path,
    ...
    #m.hparams['write_logits'] = True
    
    path = ckpt_path.split('/')[0] + '/' + ckpt_path.split('/')[1] + '/' + ckpt_path.split('/')[2]
   
    logits = pd.read_csv(os.path.join(path, 'test_results', ckpt_path.split('/')[-1][:-5], 'logits.csv'))
    #logits_merge = logits.merge(logits, on = ['latitude', 'longitude', 'year'])
    sensor = input('Sensor?')
    del_labels = len(DATASET_MERGE_SCHEMES[m.hparams['merge_scheme']]['label_names'])
    final_label = []
    if sensor == 'planet':
        ind = np.linspace(0, len(logits)-5, int(len(logits)/5))

        #info_data_merge = info_data.merge(info_data, on = ['latitude', 'longitude', 'year'])

        #all_info_merge = info_data_merge.merge(logits_merge, on = ['latitude', 'longitude', 'year'])

        #all_info_merge = all_info_merge.apply(lambda x: x+'_')
        #info_groupby = all_info_merge.groupby(['latitude', 'longitude', 'year']).sum()
        ...
        for i in ind:
            ...
            max_value5 = -1
            for j in range(del_labels):
                if float(row5[str(j)]) > max_logit5:
                    max_logit5 = float(row5[str(j)])
                    max_value5 = int(j)
            
            max_logit = max([max_logit1, max_logit2, max_logit3, max_logit4, max_logit5])
            max_ind = [max_logit1, max_logit2, max_logit3, max_logit4, max_logit5].index(max_logit)
            max_value = [max_value1, max_value2, max_value3, max_value4, max_value5][max_ind]
            final_label.append(DATASET_MERGE_SCHEMES[m.hparams['merge_scheme']]['label_names'][max_value])

    elif sensor == 'landsat':
        ind = [0]
        i = 0
        while i+4 < len(logits):
            row1 = logits.iloc[[i]]
            row2 = logits.iloc[[i+1]]
            row3 = logits.iloc[[i+2]]
            row4 = logits.iloc[[i+3]]
            ...
            list_max = [max_logit1, max_logit2, max_logit3, max_logit4, max_logit5]
            list_max_value = [max_value1, max_value2, max_value3, max_value4, max_value5]

            if float(row1['longitude']) != float(row2['longitude']) or float(row1['latitude']) != float(row2['latitude']):
                list_max = list_max[:1]
                list_max_value = list_max_value[:1]
                i = i+1
            elif float(row1['longitude']) != float(row3['longitude']) or float(row1['latitude']) != float(row3['latitude']):
                list_max = list_max[:2]
                list_max_value = list_max_value[:2]
                i = i+2
            elif float(row1['longitude']) != float(row4['longitude']) or float(row4['latitude']) != float(row4['latitude']):
                list_max = list_max[:3]
                i = i+3
                list_max_value = list_max_value[:3]
            elif float(row1['longitude']) != float(row5['longitude']) or float(row4['latitude']) != float(row5['latitude']):
                list_max = list_max[:4]
                list_max_value = list_max_value[:4]
                i = i+5
            else:
                i = i+5
            ...
            if i != len(logits):
                ind.append(i)
            max_logit = max(list_max)
            max_ind = list_max.index(max_logit)
            max_value = list_max_value[max_ind]
            final_label.append(DATASET_MERGE_SCHEMES[m.hparams['merge_scheme']]['label_names'][max_value])

        


    info_data = pd.read_csv(os.path.join(path, 'test_results', ckpt_path.split('/')[-1][:-5], 'mytest_results.csv'))
    #remove lines
    #print(ind)
    info_data = info_data.loc[ind]    
    info_data['predicted_label_fusion'] = final_label
    if os.path.exists(os.path.join(SANDBOX_DIR, 'timeseries_'+ ckpt_path.split('/')[-1][:-5])) == False:
                      os.mkdir(os.path.join(SANDBOX_DIR, 'timeseries_'+ ckpt_path.split('/')[-1][:-5]))
    
    info_data.to_csv(os.path.join(SANDBOX_DIR, 'timeseries_'+ ckpt_path.split('/')[-1][:-5], 'timeseries.csv'))
    ...
    heat_map.figure.savefig(os.path.join(SANDBOX_DIR, 'timeseries_'+ ckpt_path.split('/')[-1][:-5], 'heatmap.png'))
    ...
    heat_map_normalized.figure.savefig(os.path.join(SANDBOX_DIR, 'timeseries_'+ ckpt_path.split('/')[-1][:-5], 'heatmap_normalized.png'))
