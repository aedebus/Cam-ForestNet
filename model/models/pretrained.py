...
class CadeneModel(PretrainedModel):
    ...
    def __init__(self, model_name, model_args=None):
        ...
        num_outputs = len(model_args['labels'])
        self.fc = nn.Linear(num_ftrs, num_outputs)

    def forward(self, x):
        x = self.model.features(x['image'])
...
class TorchVisionModel(PretrainedModel):
    ...
    def __init__(self, model_fn, model_args):
        ...
        # Replace first conv with one with the correct number of channels.
        num_channels = get_num_channels(model_args)
        if num_channels != 3:
            new_conv_layer = nn.Conv2d(num_channels, NUM_INPLANES,
                                       kernel_size=7, stride=2, padding=3,
                                       bias=False)
            if "ResNet" in self.__class__.__name__:
                self.model.conv1 = new_conv_layer
            elif "DenseNet" in self.__class__.__name__:
                self.model.features.conv0 = new_conv_layer

    def forward(self, x):
        x = self.model(x['image'])
        return x
...
class Inceptionv3(TorchVisionModel):
    def __init__(self, model_args):
        super(Inceptionv3, self).__init__(models.densenet121, model_args)
